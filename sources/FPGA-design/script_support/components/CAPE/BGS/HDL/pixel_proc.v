...
module pixel_proc(
    ...
    always @(posedge pclk or negedge presetn) begin
        if (~presetn) begin
            ...
            start_pclk<=0; ack_s1<=0; ack_s2<=0; ack_stb<=0;
        end else begin
            if (wr_en) case (paddr)
                A_CTRL:   ctrl   <= pwdata;
            ...
            // Sync ack/done from ACLK
            ack_s1  <= ack_aclk;
            ack_s2  <= ack_s1;
            ack_stb <= ack_s1 && !ack_s2;
            if (ack_stb) irq_pclk <= 0;

            // 2-FF sync irq from ACLK
            irq_s1 <= irq_aclk_s;
            irq_s2 <= irq_s1;
            if (irq_s2) irq_pclk <= 1;
    ...
    always @(posedge aclk or negedge aresetn) begin
        if (~aresetn) begin
            ...
            ack_aclk<=0; irq_aclk_s<=0; busy_s<=0; done_s<=0;
            ...
            case (state)
                    ...
                    if (m_axi_rvalid && m_axi_rready) begin
                        if (beat==0) begin
                            ...
                            if (rcnt==31) begin
                                ...
                                irq_aclk_s<=1; ack_aclk<=1; state<=S_ACK;
                            end else if (pxl_done+1>=total) begin
                                result<={rbuf[30:0],((mc+{4'b0,ma}+{4'b0,mb})>MTH)};
                                ack_aclk<=1; busy_s<=0; done_s<=1; state<=S_DONE;
                            end else begin
                ...
                S_ACK: begin
                    ...
                    if (wr_en && paddr==A_CTRL && pwdata[7]) begin
    ...
    // CDC: ack_aclk and result are in ACLK domain; read via APB in PCLK domain (safe: handshake-protected)
