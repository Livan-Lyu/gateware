...
set params "NUM_TARGETS:$num_slaves \
            NUM_INITIATORS:$num_masters \
            NUM_INITIATORS_WIDTH:$num_masters_width \
            ADDR_WIDTH:38 \
            ID_WIDTH:8 \
            USER_WIDTH:4 \
            OPTIMIZATION:1 \
...
for {set i 1} {$i <= $num_accelerators} {incr i} {
    ...
    append params " TARGET${i}_START_ADDR:$base_addr"
    append params " TARGET${i}_END_ADDR:[format 0x%08x [expr {$base_addr + $base_size}]]"
    append params " TARGET${i}_DATA_WIDTH:64 TARGET${i}_TYPE:0"
    append params " INITIATOR${i}_TYPE:0 INITIATOR${i}_DATA_WIDTH:64"
}
...
for {set m 0} {$m < $num_masters} {incr m} {
    for {set s 0} {$s < $num_slaves} {incr s} {
        if {$m == 0 && $s == 0} {
            ...
            append params " INITIATOR${m}_READ_TARGET${s}:false"
            append params " INITIATOR${m}_WRITE_TARGET${s}:false"
        } elseif {$m > 0 && $s == 0} {
            # Accelerator masters can access memory (Slave0)
            append params " INITIATOR${m}_READ_TARGET${s}:true"
            append params " INITIATOR${m}_WRITE_TARGET${s}:true"
        } elseif {$m == 0 && $s > 0} {
...
for {set i 0} {$i < $num_masters} {incr i} {
    append params " INITIATOR${i}_CHAN_RS:true"
    append params " INITIATOR${i}_CLOCK_DOMAIN_CROSSING:false"
    append params " INITIATOR${i}_DWC_DATA_FIFO_DEPTH:32"
}

for {set i 0} {$i < $num_slaves} {incr i} {
    append params " TARGET${i}_CHAN_RS:true"
    append params " TARGET${i}_CLOCK_DOMAIN_CROSSING:false"
    append params " TARGET${i}_DWC_DATA_FIFO_DEPTH:32"
}

# Add additional required parameters

# Create and configure the core
create_and_configure_core -component_name {SHLS_AXI4Interconnect} -core_vlnv {Actel:DirectCore:COREAXI4INTERCONNECT:*} -params $params
