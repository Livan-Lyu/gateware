...
// SVN $Revision: 49964 $
// SVN $Date: 2025-10-25 23:17:09 +0530 (Sat, 25 Oct 2025) $
//
//
// *********************************************************************/
`timescale 1ns / 1ns
		...
		if ( AWCHAN == 0 ) 			// Address Write Channel - direct connection, no Register Slice
			...
			begin : wrch_rs_en
				...
				caxi4interconnect_RegSliceFull #
					 (  
					    .CHAN_WIDTH ( AWCHAN_WIDTH ),
					    .READY_REG  ( READY_REG    )
					 )	//  the number of channel signals to register (outside of Valid & Ready) 
				awrs (
						.sysClk	    ( 	sysClk 		),
						.arst_sync  ( 	arst_sync 	),						
						.srst_sync  ( 	srst_sync 	),						
						.mDat       (   srcAWCHAN 	),		// channel data signals to register from "initiator" or "source"
						.mValid     ( 	srcAWVALID 	),		// indicates when mDat is valid
						.mReady     ( 	srcAWREADY	),		// indicates when taking data from "initiator" or "source"
						.sDat       ( 	dstAWCHAN 	),		// channel data signals registered to "target" or "sink"
						.sValid     ( 	dstAWVALID 	),		// indicates when sDat is valid
						.sReady     ( 	dstAWREADY 	)		// indicates when target/sink taking sDat
					);
		...
		if ( ARCHAN == 0 ) 			// Address Read Channel - direct connection, no Register Slice
			...
			begin : raddrch_rs_en
				...
				arrs (
						...
						.arst_sync ( 	arst_sync 	),						
						.srst_sync ( 	srst_sync 	),						
						.mDat      (	srcARCHAN 	),		// channel data signals to register from "initiator" or "source"
						.mValid    ( 	srcARVALID 	),		// indicates when mDat is valid
						.mReady    ( 	srcARREADY	),		// indicates when taking data from "initiator" or "source"
						.sDat      (	dstARCHAN 	),		// channel data signals registered to "target" or "sink"
						.sValid    ( 	dstARVALID 	),		// indicates when sDat is valid
						.sReady    ( 	dstARREADY 	)		// indicates when target/sink taking sDat
					);
	...
	generate
		if ( RCHAN == 0 ) 			//  Read Data Channel - direct connection, no Register Slice
			...
			begin : rdatach_rs_en
				...
				// For RCHAN - initiator/src is "dst" and target/sink is "src"
				caxi4interconnect_RegSliceFull #
					( 
					    .CHAN_WIDTH ( RCHAN_WIDTH ),
					    .READY_REG  ( READY_REG   )	
					)	//  the number of channel signals to register (outside of Valid & Ready) 
				rrs (
						.sysClk	   ( 	sysClk 		),
						.arst_sync ( 	arst_sync 	),						
						.srst_sync ( 	srst_sync 	),						
						.mDat      (	dstRCHAN 	),		// channel data signals to register from "initiator" or "source"
						.mValid    ( 	dstRVALID 	),		// indicates when mDat is valid
						.mReady    ( 	dstRREADY	),		// indicates when taking data from "initiator" or "source"
						.sDat      (	srcRCHAN 	),		// channel data signals registered to "target" or "sink"
						.sValid    ( 	srcRVALID 	),		// indicates when sDat is valid
						.sReady    ( 	srcRREADY 	)		// indicates when target/sink taking sDat
					);
		...
		if ( WCHAN == 0 ) 			//  Write Data Channel - direct connection, no Register Slice
			...
			begin : wrdata_rs_en
				...
				wrs (
						...
						.arst_sync ( 	arst_sync 	),						
						.srst_sync ( 	srst_sync 	),						
						.mDat      (	srcWCHAN 	),		// channel data signals to register from "initiator" or "source"
						.mValid    ( 	srcWVALID 	),		// indicates when mDat is valid
						.mReady    ( 	srcWREADY	),		// indicates when taking data from "initiator" or "source"
						.sDat      (	dstWCHAN 	),		// channel data signals registered to "target" or "sink"
						.sValid    ( 	dstWVALID 	),		// indicates when sDat is valid
						.sReady    ( 	dstWREADY 	)		// indicates when target/sink taking sDat
					);
		...
		if ( BCHAN == 0 ) 			//  Write Response Channel - direct connection, no Register Slice
			...
			begin : wrrsp_rs_en
				...
				assign dstBCHAN = { 	dstBID, 		dstBRESP, 
										dstBUSER				 	}; 
				// For BCHAN - initiator/src is "dst" and target/sink is "src"
				caxi4interconnect_RegSliceFull #
					( 
					    .CHAN_WIDTH ( BCHAN_WIDTH ),
					    .READY_REG  ( READY_REG   )
					)	//  the number of channel signals to register (outside of Valid & Ready) 
				brs (
						.sysClk	   ( 	sysClk 		),
						.arst_sync ( 	arst_sync 	),						
						.srst_sync ( 	srst_sync 	),						
						.mDat      (	dstBCHAN 	),		// channel data signals to register from "initiator" or "source"
						.mValid    ( 	dstBVALID 	),		// indicates when mDat is valid
						.mReady    ( 	dstBREADY	),		// indicates when taking data from "initiator" or "source"
						.sDat      (	srcBCHAN 	),		// channel data signals registered to "target" or "sink"
						.sValid    ( 	srcBVALID 	),		// indicates when sDat is valid
						.sReady    ( 	srcBREADY 	)		// indicates when target/sink taking sDat
					);
