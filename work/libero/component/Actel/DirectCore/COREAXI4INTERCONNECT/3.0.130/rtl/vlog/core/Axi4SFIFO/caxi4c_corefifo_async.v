...
// SVN $Revision: 44151 $
// SVN $Date: 2023-09-22 20:34:12 +0530 (Fri, 22 Sep 2023) $
//
// Resolved SARs
...
module caxi4c_corefifo_async (
   ...
   // --------------------------------------------------------------------------
   parameter AE_DYN_EN        = 1;
   parameter AF_DYN_EN        = 1;
   ...
   input  [WRITE_DEPTH-1:0]    afull_in_dyn_thold;     // almost full dynamic threshold status flag
   output [WRITE_DEPTH:0]      wrcnt;                  // number of elements remaining in write domain
                                                        
   output                      empty;                  // empty status flag
   output                      aempty;                 // almost empty status flag
   input  [READ_DEPTH-1:0]     aempty_in_dyn_thold;    // almost empty dynamic threshold status flag
   ...
   reg  [WRITE_DEPTH : 0]         wptr_gray /* synthesis syn_safe_cdc = 1*/;
   reg  [READ_DEPTH  : 0]         rptr_gray /* synthesis syn_safe_cdc = 1*/;
   reg  [READ_DEPTH  : 0]         rptr_gray_fwft /* synthesis syn_safe_cdc = 1*/;
   reg                            empty_r_fwft;
   reg                            aempty_r_fwft;
   reg                            full_reg;
   reg                            empty_reg;
   ...
   reg  [WRITE_DEPTH:0]           afthreshi;
   reg  [READ_DEPTH:0]            aethreshi;
   ...
   // DCFIFO RTL Enhancement for performance
   if( HIGH_FREQUENCY == 1) begin
...
endgenerate
   ...
   // -------------------------------------------------------------------------- 
    ...
    always@(*)
	  begin
	     if(AF_FLAG_STATIC)
		    afthreshi    =   (AFULL_VAL-1);
		 else if (AF_DYN_EN)
		    afthreshi    =  afull_in_dyn_thold- 1; 
		 else
		    afthreshi    =  FULL_WRITE_DEPTH;
	  end
	  
    always@(*)
	  begin
	     if(AE_FLAG_STATIC)
		    aethreshi    =   AEMPTY_VAL;
		 else if (AE_DYN_EN)
		    aethreshi    =  aempty_in_dyn_thold; 
		 else
		    aethreshi    =  2;
	  end
